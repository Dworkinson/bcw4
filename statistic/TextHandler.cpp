...
std::string TextHandler::tempWord;
...
void TextHandler::insert(const std::string& word, std::set<std::string>* lst) {
    lst->insert(word);
}

void TextHandler::insert(const std::string& word, std::map<std::string, int>* lst) {
...
void TextHandler::compareString(char symbol) {
    int diff = 'a' - 'A';
    if ( isLetter(symbol) || symbol == '\'' ) {
        if ( symbol < 'a' && symbol != '\'') {
                symbol += diff;
            }
        TextHandler::tempWord += symbol;
    } else {
        if ( TextHandler::tempWord.empty() ) { return; }
        insert(TextHandler::tempWord, words);
        insert(TextHandler::tempWord, wordsStatistics);
        TextHandler::tempWord.clear();
    }
}
