...
void Category::validate(const std::string& category)
{
	auto search = Category::categories.find(category);
...
Category::Category(const std::string& category) : category(category)
{
	...
	Category::categories.emplace(category);
}
...
void Category::showProduction()
{
	...
	std::cout << "============================" << std::endl;
	std::cout << "Products in category \"" << this->category << "\": " << std::endl;
